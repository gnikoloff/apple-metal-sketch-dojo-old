...
//extension FPSCamera: Movement {}

struct ArcballCamera: Camera {
  static let UP_VECTOR: float3 = [0, 1, 0]
  ...
  var viewMatrix: float4x4 {
    ...
    if target == position {
      ...
      matrix = float4x4(eye: position, center: target, up: ArcballCamera.UP_VECTOR)
    }
    return matrix
  }
  ...
  mutating func update(deltaTime: Float) {
    let input = InputController.shared
    let scrollIntensity = Settings.mouseScrollSensitivity
    distance -= (input.mouseScroll.x + input.mouseScroll.y) * scrollIntensity
